        ...
        with st.spinner(f'Fetching data for {ticker}…'):
            try:
                ...
                with c2:
                    ...
                    car_pct10 = df_history['car_3day'].quantile(0.10)
                    car_pct90 = df_history['car_3day'].quantile(0.90)
                    bar_col   = "#22c55e" if car_pred > 0 else "#ef4444"

                    # Use a horizontal bullet-style chart so tiny values always show
                    fig_car = go.Figure()

                    # Background band: 10th–90th percentile range of historical CARs
                    fig_car.add_shape(
                        type="rect",
                        x0=car_pct10, x1=car_pct90,
                        y0=0.2, y1=0.8,
                    ...
                    # Prediction needle — always at least 0.5% wide for visibility
                    needle_x1 = car_pred if abs(car_pred) > 0.005 else (0.005 if car_pred >= 0 else -0.005)
                    fig_car.add_shape(
                        type="rect",
                        x0=0, x1=needle_x1,
                        y0=0.32, y1=0.68,
                    ...
                    fig_car.add_annotation(
                        x=needle_x1,
                        y=0.5,
                        text=f"<b>{car_pred:.2%}</b>",
                        showarrow=True,
                        arrowhead=2,
                        arrowcolor=bar_col,
                        arrowsize=0.8,
                        arrowwidth=1.5,
                        ax=40 if car_pred >= 0 else -40,
                        ay=0,
                        font=dict(size=14, color=bar_col, family='Syne'),
                        bgcolor='rgba(0,0,0,0)',
                    )
                    # Std dev label
                    fig_car.add_annotation(
                        x=car_pct10, y=0.12,
                        text=f"10th pct: {car_pct10:.1%}",
                        showarrow=False,
                        font=dict(size=9, color='#555', family='DM Mono'),
                        xanchor='left'
                    )
                    fig_car.add_annotation(
                        x=car_pct90, y=0.12,
                        text=f"90th: {car_pct90:.1%}",
                        showarrow=False,
                        font=dict(size=9, color='#555', family='DM Mono'),
                        xanchor='right'
                    )

                    axis_range = max(abs(car_pct10), abs(car_pct90), abs(car_pred) * 2, 0.05)
                    fig_car.update_layout(
                        **PLOT_LAYOUT,
                        ...
                        showlegend=False,
                        xaxis=dict(
                            range=[-axis_range * 1.2, axis_range * 1.2],
                            tickformat='.0%',
                            gridcolor='#1a1a1a',
